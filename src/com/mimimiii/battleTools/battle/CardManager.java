 ...
 */
    ...
    public CardType createCardType(String name) {
        if (name.toLowerCase().contains("spell_waterspell")) {
            ...
            return CardType.ORK;
        } else if (name.toLowerCase().contains("wizzard")) {
    ...
    public Deck getDeckUser(User user) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                cards.add(new Card(rs.getString(1), name, rs.getFloat(3), createCardType(name), createPackage(name)));
            }
            deck = new Deck(cards);
            rs.close();
            conn.close();
